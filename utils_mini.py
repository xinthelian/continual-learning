...
def smaller_dataset(dataset, size):
    data = dataset[0][0]
    labels = []
    for i in range(1, size):
        data = torch.cat((data, dataset[i][0]), dim=0)
    ...
    dataset = SmallerDataset(data, labels)
    return dataset
...
def get_data_loader(dataset, overall_size, batch_size, cuda=False, drop_last=False, augment=False):
    ...
    dataset_ = smaller_dataset(dataset_, overall_size)
    # print(dataset_[12])
