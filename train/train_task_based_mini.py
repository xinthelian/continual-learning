...
import tqdm
import copy
from utils_mini import get_data_loader,checkattr
...
def train_cl(model, train_datasets, iters=2000, batch_size=32, baseline='none',
    ...
    for context, train_dataset in enumerate(train_datasets, 1):
        ...
        for batch_index in range(1, iters_to_use+1):
            ...
            # # ====== MINI UPDATE ======
            # training_dataset = smaller_dataset(train_dataset)

            # Update # iters left on current data-loader(s) and, if needed, create new one(s)
            iters_left -= 1
            if iters_left==0:
                data_loader = iter(get_data_loader(training_dataset, TRAIN_SIZE, batch_size, cuda=cuda, drop_last=True))
                    ...
                    for context_id in range(up_to_context):
                        ...
                        if iters_left_previous[context_id]==0:
                            data_loader_previous[context_id] = iter(get_data_loader(
                                previous_datasets[context_id], TRAIN_SIZE, batch_size_to_use, cuda=cuda, drop_last=True
                            ))
...
def train_fromp(model, train_datasets, iters=2000, batch_size=32,
    ...
    for context, train_dataset in enumerate(train_datasets, 1):
        ...
        for batch_index in range(1, iters+1):
            ...
            iters_left -= 1
            if iters_left==0:
                data_loader = iter(get_data_loader(train_dataset, TRAIN_SIZE, batch_size, cuda=cuda, drop_last=True))
                iters_left = len(data_loader)
...
def train_gen_classifier(model, train_datasets, iters=2000, epochs=None, batch_size=32,
    ...
    for class_id, train_dataset in enumerate(train_datasets):
        ...
        if epochs is not None:
            data_loader = iter(get_data_loader(train_dataset, TRAIN_SIZE, batch_size, cuda=cuda, drop_last=False))
        ...
        for batch_index in range(1, iters+1):
            ...
            iters_left -= 1
            if iters_left==0:
                data_loader = iter(get_data_loader(train_dataset, TRAIN_SIZE, batch_size, cuda=cuda,
